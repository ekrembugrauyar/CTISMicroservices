...
var booksDbConnectionString = builder.Configuration.GetConnectionString("BooksDB");
builder.Services.AddDbContext<BooksDb>(options =>
    options.UseNpgsql(booksDbConnectionString));

builder.Services.AddMediatR(config => config.RegisterServicesFromAssembly(typeof(BooksDbHandler).Assembly));
...
logger.LogInformation("Books API JWT Configuration:");
...
builder.Services.AddSwaggerGen(c =>
{
    ...
    c.SwaggerDoc("v1", new OpenApiInfo
    {
        Title = "Books API",
        Version = "v1",
        Description = "API for managing books and related operations"
    });
